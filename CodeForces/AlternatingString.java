...
/*
 *   Author  : Aritra Dutta
 *   Created : Sunday, 01.09.2024  09:16 pm
 */
...
public class AlternatingString {
    ...
    private static int minOperations(int n, String s) {
        ...
        if (n % 2 == 1) {
            int result = Math.min(minOperations(n - 1, s.substring(1)), minOperations(n - 1, s.substring(0, n - 1)));
            for (int i = 1; i < n - 1; i++) {
                String temp = s.substring(0, i) + s.substring(i + 1);
                result = Math.min(result, minOperations(n - 1, temp));
            }
            return result;
        }

        char evenChar = s.charAt(0);
        char oddChar = s.charAt(1);
        int evenChanges = 0;
        int oddChanges = 0;
